...
4. 구독 이벤트 (Subscribe to events)
   GitHub 저장소 Settings > Webhooks 화면에서는
   "Which events would you like to trigger this webhook?" 항목에서
   "Let me select individual events."를 선택한 뒤 아래 이벤트만 체크하세요.
   "Just the push event."는 리뷰 요청 트리거가 아니고,
   "Send me everything."은 불필요한 이벤트까지 전송합니다.

   - Pull requests: opened / synchronize / reopened / ready_for_review 액션에서 리뷰 시작
   - Issue comments: PR 코멘트에서 리뷰어 멘션 시 후속 응답(follow-up) 처리

   비슷한 이름이지만 아래 이벤트는 이 서버 트리거가 아니므로 체크하지 않습니다.
