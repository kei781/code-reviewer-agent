...
| 오케스트레이터 | GitHub Actions | 이벤트 감지, 정책 확인, 상태 기록, loop 제어, merge gate |
...
|---|---|---|---|
| P0 | Reviewer Signal MVP | 자동 1차 리뷰, 후속 질의, same-repo guard, `MERGE_SIGNAL` | 사람 |
...
## FR-001. P0 Reviewer 자동 1차 리뷰

### 설명

PR이 생성되거나 새 커밋이 push되면 reviewer model R이 PR diff를 리뷰하고, formal PR review가 아닌 structured PR comment를 생성하거나 업데이트한다.
...
- workflow가 `main`에 존재한다.
- PR 변경 파일이 정책상 read 가능 범위다.

### Reviewer 입력

- PR title/body
- PR diff 또는 changed files summary
- repository 기준 문서
  - `ADR.md`
  - `PRD.md`
...
Round <N> · Blockers this round: <K> (prev <K-1>)
...
- PR 생성 시 reviewer summary comment가 생성된다.
- PR 업데이트 시 새 head SHA 기준으로 재리뷰된다.
- 동일 head SHA에는 중복 리뷰하지 않는다.
- `MERGE_SIGNAL`이 명확히 표시된다.
- blocker와 suggestion이 분리된다.
- `PASS`는 unresolved blocker가 0일 때만 표시된다.
- suggestion은 PASS를 막지 않는다.
- formal approve 또는 formal PR review 제출을 필수 전제로 삼지 않는다.
- failure 또는 skip 사유가 workflow log 또는 PR comment에 남는다.
...
P0/P1에서는 branch protection이 없어도 reviewer workflow는 도입할 수 있지만, auto-merge는 도입하지 않는다.

---

## 12. 파일 구조

파일명은 role 중심으로 둔다. 특정 vendor/action은 workflow 내부 adapter 설정으로만 표현한다.

### P0 필수

```text
.github/workflows/
  ai-reviewer-review.yml
  ai-reviewer-interactive.yml

.github/ai/
  prompts/
    reviewer-review.md
    reviewer-interactive.md
```
...
### P0 — Reviewer Signal MVP

1. reviewer adapter 인증 secret 등록
2. reviewer GitHub App 또는 action 인증 설정
3. `.github/workflows/ai-reviewer-review.yml` 추가
4. `.github/workflows/ai-reviewer-interactive.yml` 추가
5. same-repo/draft/closed guard 추가
6. last-reviewed-sha marker 구현
7. `reviewer-review.md` prompt 작성
8. `MERGE_SIGNAL` 형식 고정
9. workflow failure/skip log 확인
10. test PR로 review comment 형식 검증
