...
| 서버 책임 | webhook 수신 · git clone/fetch/head-SHA checkout · **격리 샌드박스에서 에이전트 실행** · 게시용 scoped 토큰 발급 · dedup/guard(결정론) |
| 코드 세팅 | diff API가 아니라 **head SHA 고정 체크아웃**(clone+fetch+detached checkout) — 에이전트가 레포 전체 문맥 확보 |
| 모델 | Reviewer-A=Claude, Reviewer-B=Codex (둘 다 **리뷰어**; 기존 "F=Fixer" 아님) |
| 게시 주체 | **서버측 GitHubPort** (에이전트는 findings JSON만 반환, 직접 게시 안 함) |
| git fetch 주체 | **서버측** (clone/fetch/head-SHA checkout은 서버; 샌드박스엔 읽기전용 작업공간만) |
...
3. 서버가 격리 작업공간에 git clone → git fetch --no-tags origin <branch> → git checkout --detach <head-sha> 로 코드베이스 세팅
...
단, D4 "독립적 실패 모드" 중 **판정 중립성**은 부분 완화됨을 ADR 개정안에 명시한다: Claude가 리뷰어 A이자 교차검증자를 겸하므로 **생성 단계는 독립이나 판정 단계엔 잔여 편향**이 있다(§6.3 완화책 적용). 완전 중립이 필요하면 교차검증을 서버측 분리 reconcile로 승격(§12).

### 문서 갱신
`directory-structure.md`에 server/webhook/sandbox/agent-runner/git 어댑터 반영(규칙 7). `docs/phase-plan.md`는 아직 rev1(GHA·자동수정·수렴) 형태이므로, **같은 선행 PR에서 앵상블 모델로 재도출하거나 superseded로 명시**한다 (저장소에 모순된 phase 맵이 공존하지 않도록).
...
│      2) GitWorkspace(서버측): clone+fetch+head-SHA checkout → 읽기전용 작업공간│
│      3) SandboxRunner: 작업공간(RO)+모델토큰만 주입, 에이전트 실행 ─┼──┐
│         (GitHub 토큰·App key 미주입)                               │  │
│      4) findings JSON 수신 → dedup → 서버가 인라인+요약 게시        │  │
│  state: [StateStorePort(SQLite)]  (jobs, processed_events,        │  │
│         pr_review_state: last_reviewed_sha, posted finding keys)  │  │
└──────────────────────────────────────────────────────────────────┘  │
   App key·게시·fetch=서버측      ▼ (샌드박스: 읽기전용 PR코드 + 모델토큰만)
        │                  ┌─────────────────────────────────────────┐
        ▼                  │ 격리 컨테이너 (일회용, egress=모델 API만)│
   GitHub API              │  Orchestrator (MVP=Claude Code · 교체가능)│
   (인라인·요약·라벨)      │   ├─spawn▶ Reviewer-A: Claude (fresh)     │
                           │   ├─spawn▶ Reviewer-B: Codex  (fresh)     │
                           │   │        A/B 상호 trace 비공유          │
                           │   └─ 코드기반 교차검증 → findings(JSON)   │
                           └─────────────────────────────────────────┘
```
...
- `GitWorkspacePort`(clone/fetch/head-SHA checkout, 작업공간 생성·파기)
- `OrchestratorPort`(독립 Reviewer 패스들을 spawn → findings 수집 → 코드기반 교차검증 조율. **독립성을 구조로 강제하는 seam** — MVP 구현=Claude Code, 추후 중립 reconcile로 교체)
- `ReviewerPassPort`(단일 모델 1회 fresh-context 리뷰 → findings JSON. 다른 패스와 trace 공유 안 함)
- `SandboxRunnerPort`(격리 컨테이너 실행; 입력=PR 메타+체크리스트+**model_tokens만**(GitHub 토큰 제외), 출력=findings JSON)
- `StateStorePort`, `QueuePort`, `Clock/Logger`

**adapters**
- `GitHubAppAdapter`(Octokit + 설치토큰 발급 + webhook HMAC)
- `GitCliAdapter`(git clone/fetch/head-SHA checkout — hooks·filter 무력화)
- `ContainerSandboxAdapter`(예: docker/ephemeral; 격리 launch, egress allowlist 강제)
- `ClaudeCodeOrchestratorAdapter`(**MVP 오케스트레이터**: `OrchestratorPort` 구현. Reviewer 패스 spawn + 코드기반 교차검증. 교체 대상 — 추후 `ServerReconcileOrchestrator`)
- `ClaudeReviewerPassAdapter` / `CodexReviewerPassAdapter`(각각 fresh-context 단일 모델 리뷰 → findings)
...
작업공간을 "읽기전용"으로 샌드박스에 주입 + 모델 토큰만 전달 (GitHub 토큰·App key 미주입)
  ▼
SandboxRunner: 격리 컨테이너에서 Orchestrator(MVP=Claude Code) 실행
  → Orchestrator가 Reviewer-A(Claude)·Reviewer-B(Codex)를 각각 fresh 패스로 spawn (상호 trace 비공유)
  → 두 findings를 "코드베이스 참고" 교차검증 → 유효 findings JSON 반환
  ▼
서버가 findings dedup(이미 게시한 fingerprint 제외) → 서버측 GitHubPort로
  인라인 코멘트(심각도 태그) + 요약 1건 게시 (게시 토큰은 샌드박스에 들어가지 않음)
  ▼
last_reviewed_sha·posted finding keys 기록
```

> **풀 체크아웃과 diff API를 둘 다 쓰는 이유**: 체크아웃(작업공간)은 에이전트가 레포 전체 문맥으로 리뷰하기 위함이고, diff API는 서버측 guard 입력(risky path·변경파일·입력한도)과 인라인 코멘트의 정확한 라인 앵커링에 쓴다. 서로 역할이 다르다.

### 6.3 독립성 + 코드베이스 그라운딩 프로토콜 (ADR D4 / AGENTS 규칙 4 — 구조로 강제)
**독립성을 프롬프트가 아니라 구조로 강제한다.** Reviewer-A·Reviewer-B는 `OrchestratorPort`가 **각각 별도 fresh-context 패스로 spawn**하는 독립 모듈이다. 두 패스는 서로의 리뷰·추론 trace를 보지 못하고, 각자 **체크아웃된 코드베이스 전체** + PR diff + 체크리스트만 입력받아 독립 findings를 낸다(병렬/순차, 교차 노출 없음). Codex는 `codex exec` 류 fresh context라 Claude 세션 transcript를 공유하지 않는다. → **이 생성 단계 분리가 AGENTS 규칙 4(trace 비공유)를 충족**한다.
- **교차검증은 반드시 실제 코드베이스를 참고(필수).** Orchestrator가 A·B findings 출력을 받아 각 finding마다 **인용된 파일·라인을 실제로 열어** ① 코드 실증 ② 수정 제안의 문맥 타당성 ③ 합의/중복/오탐을 판정하고, **코드로 뒷받침되는 유효 finding만** 통과시킨다(환각·오독 기각). 레포·브랜치 체크아웃의 핵심 이유가 이 코드 그라운딩이다.
- 독립성 무충돌: 공유하는 건 추론 trace가 아니라 **공통 ground truth인 코드**뿐이다.
- ⚠️ **MVP 잔여편향(문서화·교체가능)**: MVP의 `OrchestratorPort` 구현은 Claude Code다. Reviewer-A·B는 별도 fresh 패스라 **생성 단계 독립성은 보장**되지만, 오케스트레이터(Claude Code)가 교차검증(판정)을 겸하므로 **판정 단계 잔여편향**은 남는다. 이는 **포트 뒤 어댑터 교체 사안** — 추후 `ServerReconcileOrchestrator`(중립 reconcile)로 갈아끼우면 판정 중립성까지 확보. (AGENTS.md v5 노트에 이 예외를 명시)
...
- `dedupe_key` = `"{type}:{repo}#{pr}@{head_sha}"` 형식으로 결정(같은 SHA·타입 중복 잡 방지).
- `posted_finding_keys`는 PR open(또는 epoch) 단위로 정리해 무한 증가를 막는다.
...
- **GitHub 자격증명 샌드박스 미노출**: App private key·설치토큰은 **서버에만**. clone/fetch/head-SHA checkout도 서버측에서 수행하고, 게시(인라인·요약)도 서버측 GitHubPort가 수행. 따라서 **샌드박스에는 GitHub 토큰이 아예 들어가지 않는다** (공격면 축소; 다이어그램의 "모델토큰만" 경계가 실제와 일치).
...
> P0a/P0b는 **이 빌드 내부 단계명**이며 ADR/PRD의 P0/P1과 다르다.

**P0a — 단일 패스 리뷰 (서버 기반 골격)**
0. **(완료, 이 PR)** ADR.md §0 · PRD.md §0 · directory-structure v5 · phase-plan v5 개정 + **AGENTS.md v5 노트**(오케스트레이터·독립성). 코딩은 이 PR 머지 후.
1. HttpWebhookServer + HMAC + processed_events
2. SqliteQueue + Worker + lease 복구
3. GitHubAppAdapter(설치토큰 발급, diff/labels, 인라인 코멘트 upsert, 요약) — 게시·fetch 서버측
4. GitCliAdapter(clone/fetch/head-SHA checkout) — **git hooks·filter 무력화** + 일회용 작업공간
5. ContainerSandboxAdapter(격리, **egress allowlist 강제, GitHub 토큰·App key 미주입, PR 에이전트 설정 중화**) + `model_tokens`만 주입
6. `OrchestratorPort` + `ClaudeCodeOrchestratorAdapter`(MVP) + `ClaudeReviewerPassAdapter`(단독 fresh 리뷰 → findings JSON)
7. ReviewFinding 스키마/파서/렌더러 + Policy guard(same-repo/draft/fork/risky/한도) + Dedup(SHA·fingerprint·dedupe_key)
8. RunEnsembleReview(단일 패스 버전) + pr_review_state
9. 실패/스킵 코멘트 + 구조화 로그

**P0b — Codex 추가 + 교차검증 (앵상블 완성)**
1. `CodexReviewerPassAdapter`(fresh context 실행 보장)
2. Orchestrator가 A/B를 독립 spawn(§6.3) + **코드기반 교차검증** 단계
3. 유효 findings만 게시 + 합의 표기 요약

**향후(future scope)**: 중립 `ServerReconcileOrchestrator`로 판정 중립성 확보, 사람 트리거 옵트인 자동 수정(rev1의 Fixer/apply/수렴), P2 verdict check·auto-merge — ADR 로드맵대로.

---

## 12. 오픈 이슈

- **교차검증 중립성 (결정됨)**: 독립성은 `OrchestratorPort`로 구조 강제(A/B는 별도 fresh 패스). MVP 오케스트레이터=Claude Code라 판정 단계 잔여편향은 남으나 어댑터 교체 사안 — 중립 `ServerReconcileOrchestrator` 도입 "시점"만 운영 후 결정.
