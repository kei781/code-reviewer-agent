...
SandboxRunner: 격리 컨테이너에서 에이전트 세션 실행
  → 독립 리뷰 A(Claude) + 독립 리뷰 B(Codex) → "코드베이스 참고" 교차검증 → 유효 findings JSON 반환
  ▼
...
### 6.3 독립성 + 코드베이스 그라운딩 프로토콜 (ADR D4 / AGENTS 규칙 4 준수)
- 패스 A(Claude)와 패스 B(Codex)는 **서로의 리뷰·추론 trace를 보지 못한다.** 각자 **체크아웃된 코드베이스 전체** + PR diff + 체크리스트를 입력으로 독립 findings를 낸다(병렬 또는 순차, 단 교차 노출 없음).
- Codex는 fresh context(`codex exec` 류)에서 실행되어 Claude 세션의 transcript를 공유하지 않는다.
- **교차검증은 반드시 실제 코드베이스를 참고해 수행한다 (필수).** A·B의 findings 출력을 받되, 각 finding마다 **인용된 파일·라인을 실제로 열어** ① 코드로 실증되는가 ② 제안 수정이 문맥상 타당한가 ③ 양쪽 합의/중복/오탐 여부를 판정하고, **코드로 뒷받침되는 유효 finding만** 통과시킨다. 코드 근거가 없는 지적(환각·오독)은 기각한다.
  - **레포·브랜치를 가져와 체크아웃하는 핵심 이유가 바로 이 코드 그라운딩이다** — 텍스트 findings만 비교하지 않는다.
  - 독립성과 무충돌: A/B가 공유하는 건 서로의 "추론 trace"가 아니라 **공통 ground truth인 코드**뿐이다 (코드 참고 ≠ trace 공유).
...
- **Egress allowlist**: 샌드박스 네트워크는 **모델 API(api.anthropic.com / openai 등)만** 허용. GitHub fetch·게시가 서버측이라 샌드박스엔 github 접근이 불필요 → allowlist 최소. 그 외 차단으로 exfil 방지.
