...
| git fetch 주체 | **서버측** (clone/checkout/pull은 서버; 샌드박스엔 읽기전용 작업공간만) |
...
│      2) GitWorkspace(서버측): clone+checkout+pull → 읽기전용 작업공간│
...
- `GitCliAdapter`(git clone/checkout/pull — hooks·filter 무력화)
...
4. GitCliAdapter(clone/checkout/pull) — **git hooks·filter 무력화** + 일회용 작업공간
