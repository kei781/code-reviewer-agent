...
This document records the corrected P0 runtime shape requested after the initial GitHub Actions-centered scaffold. The canonical P0 path is a **webhook-driven review server** that prepares a local checkout and then runs a Claude Code orchestrator with Claude Code and Codex reviewer agents.
...
0. **Preconfiguration**
   - Install Codex.
   - Install Claude Code.
   - Connect the Codex plugin/tooling into Claude Code so the orchestrator can invoke Codex as an independent reviewer.

1. **GitHub PR event**
   - A PR is created for a specific branch.
...
4. **Orchestrated independent reviews**
   ...
   - The orchestrator gives both reviewers this contract: review the PR independently, then cross-validate only after both candidate reviews are complete.

5. **Codebase-backed cross-validation**
   - During cross-validation, the orchestrator must reopen and inspect the checked-out files for every candidate finding.
   - A finding is publishable only when it has concrete file/line evidence from the local checkout.
   - The orchestrator drops stale, duplicate, speculative, or non-reproducible findings.

6. **Publish review comments**
   - After validation, the orchestrator posts only valid findings as PR review comments, preferably inline on the relevant code.
...
Future agents must preserve this same-level pairing so that module responsibility and prompt/harness contract are easy to audit together.
